...
public class MagneYStrategy implements SignalStrategy {
    ...
    public Bitmap generateSignalChart(Context context, People people, DataPeople dato, Sennales sennales) {
        ...
        ArrayList<Float> sennalesArrayList = manager.SennalesMismaFechaMagneY(people.getCi(), dato, sennales);
        ArrayList<Entry> values = new ArrayList<>();
        for (int i = 0; i < sennalesArrayList.size(); i++) {
            values.add(new Entry(i, sennalesArrayList.get(i)));
        }
        LineDataSet dataSet = new LineDataSet(values, "Magnetómetro  en Y");
